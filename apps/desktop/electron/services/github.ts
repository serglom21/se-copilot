  ...
  async createRepoAndPush(
    ...
    try {
      ...
      if (isUpdate && fs.existsSync(gitDir)) {
        ...
        // Add remote without token (for security)
        const remoteUrl = `https://github.com/${settings.github.username}/${repoName}.git`;
        await git.addRemote('origin', remoteUrl);

        // For push, use authenticated URL temporarily
        const authUrl = `https://${settings.github.accessToken}@github.com/${settings.github.username}/${repoName}.git`;

        // Push to main branch (force if repo already exists on GitHub)
        if (isUpdate) {
          await git.push(authUrl, 'main:main', ['--force', '-u']);
          console.log('✓ Force pushed to existing repo');
        } else {
          await git.push(authUrl, 'main:main', ['-u']);
